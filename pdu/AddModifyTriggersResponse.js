...
class PduAddModifyTriggerResponse {
    constructor({
        resultCode = "Success",
    ...
    static fromData(data) {
        ...
        const reportTypeBits = binaryString.slice(15, 17);
        const { reportType } = ElementReportType.fromValue(reportTypeBits);

        const type5ElementsBits = binaryString.slice(17);
        const type5Elements = parseType5Elements(type5ElementsBits);
        
        return new PduAddModifyTriggerResponse({
            pduType,
            pduTypeExtension,
            requestResponse,
            resultCode,
    ...
    toData(){
        ...
        const binaryString = pduType + pduTypeExtension + requestResponse + resultCode + reportType + type5Elements;
        const hexString = binaryToHex(binaryString);

        return hexString;
    }
}
module.exports = PduAddModifyTriggerResponse;